...
import type { HandicraftBookingDto } from '@/apis/__generated/model/dto'
import { Delete, Edit, Plus } from '@element-plus/icons-vue'
import { useTableHelper } from '@/components/base/table/table-helper'
import { useTagStore } from '@/layout/store/tag-store'

const tagStore = useTagStore()
type HandicraftBookingScope = Scope<
  HandicraftBookingDto['HandicraftBookingRepository/COMPLEX_FETCHER_FOR_ADMIN']
>
const handicraftBookingTableHelper = inject(
  'handicraftBookingTableHelper',
  useTableHelper(
...
const handleDelete = (ids: string[]) => {
  ElMessageBox.confirm('此操作将删除数据且无法恢复, 是否继续?', '警告', {
    ...
    api.handicraftBookingForAdminController.delete({ body: ids }).then((res) => {
...
<template>
  <div>
    ...
    >
      ...
      <el-table-column label="日期" prop="date" sortable="custom" show-overflow-tooltip width="120">
        <template v-slot:default="{ row }: HandicraftBookingScope">
          {{ row.date }}
        </template>
      </el-table-column>
