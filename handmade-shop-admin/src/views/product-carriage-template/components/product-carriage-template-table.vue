...
import type { ProductCarriageTemplateDto } from '@/apis/__generated/model/dto'
import { Delete, Edit, Plus } from '@element-plus/icons-vue'
import { useTableHelper } from '@/components/base/table/table-helper'
import { useTagStore } from '@/layout/store/tag-store'

const tagStore = useTagStore()
type ProductCarriageTemplateScope = Scope<
  ProductCarriageTemplateDto['ProductCarriageTemplateRepository/COMPLEX_FETCHER_FOR_ADMIN']
>
const productCarriageTemplateTableHelper = inject(
  'productCarriageTemplateTableHelper',
  useTableHelper(
...
const handleDelete = (ids: string[]) => {
  ElMessageBox.confirm('此操作将删除数据且无法恢复, 是否继续?', '警告', {
    ...
    api.productCarriageTemplateForAdminController.delete({ body: ids }).then((res) => {
  ...
  <div>
    ...
    >
      ...
      <el-table-column
        ...
        width="120"
      >
        <template v-slot:default="{ row }: ProductCarriageTemplateScope">
          {{ row.name }}
        </template>
      </el-table-column>
      <el-table-column
      ...
      <el-table-column label="" prop="valid" sortable="custom" show-overflow-tooltip width="120">
        <template v-slot:default="{ row }: ProductCarriageTemplateScope">
